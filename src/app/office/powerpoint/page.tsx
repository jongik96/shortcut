...
export default function PowerPointPage() {
  ...
  // PowerPoint 관련 단축키만 필터링 (플랫폼별)
  const powerpointShortcuts = useMemo(() => {
    return shortcuts.filter(shortcut => 
      shortcut.software === 'powerpoint'
    ).map(shortcut => ({
      ...shortcut,
  ...
  const filteredShortcuts = useMemo(() => {
    let filtered = powerpointShortcuts;
    ...
    return filtered;
  }, [powerpointShortcuts, selectedCategory, searchQuery]);
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-8">
          ...
          <div className="lg:col-span-3">
            {/* Page Description */}
            <div className="mb-8">
              <h2 className="text-3xl font-bold text-gray-900 mb-4">PowerPoint のショートカット</h2>
              <p className="text-lg text-gray-600 mb-6">
                PowerPoint でプレゼンテーション作成を効率化するショートカットを学びましょう。
