...
export default function GoogleDocsPage() {
  ...
  const filteredShortcuts = useMemo(() => {
    return shortcuts
      .filter(shortcut => shortcut.software === 'docs')
      .filter(shortcut => {
  ...
  const categories = useMemo(() => {
    const categorySet = new Set(shortcuts.filter(s => s.software === 'docs').map(s => s.category));
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-8">
          ...
          <div className="lg:col-span-3">
            <div className="mb-8">
              <h1 className="text-3xl font-bold text-gray-900 mb-2">Google Docs</h1>
              <p className="text-gray-600">Google Docsの便利なショートカットキーを学びましょう</p>
            </div>
