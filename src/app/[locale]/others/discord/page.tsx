...
export default function DiscordPage() {
  ...
  const filteredShortcuts = useMemo(() => {
    return shortcuts
      .filter(shortcut => shortcut.software === 'discord')
      .filter(shortcut => {
  ...
  const categories = useMemo(() => {
    const categorySet = new Set(shortcuts.filter(s => s.software === 'discord').map(s => s.category));
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-8">
          ...
          <div className="lg:col-span-3">
            <div className="mb-8">
              <h1 className="text-3xl font-bold text-gray-900 mb-2">Discord</h1>
              <p className="text-gray-600">Discordの便利なショートカットキーを学びましょう</p>
            </div>
